...
| **The subscription lifecycle after signup is unbuilt** — `docs/scheduled/subscription-lifecycle.md` | **B5 is the precondition; this is what the endpoint still cannot do once registered.** `checkout.session.completed` is the *only* event handled — no `invoice.paid`, no `invoice.payment_failed`, no `customer.subscription.deleted`. So **a client whose card fails, or who cancels in Stripe, keeps their full tier indefinitely**: nothing degrades, nothing alerts, no term is written, and GC finds out by noticing the money stopped. The schema already describes the whole lifecycle and cannot reach it — **only `signup` of the six `term_trigger_enum` values is ever written**, `org_status = 'payment_lapsed'` has **no writer at all** (though `page.tsx:25` renders a label for it), and `subscriptions.current_period_end` is written once and never refreshed, so it silently goes stale at the first renewal. Separately and before the revenue module: rule 8 says *"the math reads terms only"*, and with no `renewal` term `contract_terms` shows one `signup` row forever — statements built on it would be confidently wrong. Needs a **lapse cron**, and there is no scheduled-job infrastructure of any kind today (no `supabase/functions`, no `vercel.json`). Rule 11 constrains the fix: lapse must gate at the point of action, **never a nightly sweep**, or it auto-takes-down live titles |
...
| **Account-token cleanup** | **The repo secret was deleted 2026-07-27, which closes the real exposure** — a token reachable by any workflow run. Eight account tokens exist on the account; the dashboard names them, so they CAN be told apart. **Safe to revoke on evidence, now:** `SUPABASE_ACCESS_TOKEN` (`sbp_4764…30ad`, **Never used** — a live credential that has authenticated nothing, no expiry until 06 Aug 2026) and `claude-cli` (`sbp_73a2…8073`, already **Expired**). **Probably this repo but UNVERIFIED:** `github-ci` (`sbp_3e24…5532`, last used 18 hours ago, matching this morning's CI runs). The name and timing fit, but nothing here can see another repo's secrets — confirm no other repo holds a `SUPABASE_ACCESS_TOKEN` secret before revoking it. `royalogic-ci-deploy` is separately named, which is evidence each CI has its own token. **Keep:** `cli_…178460…` (this machine, used 2h ago) and `royalogic-ci-deploy`. Three stale `cli_…` logins (11d / 22d / 1mo) are cleanup, not security. **Six of the eight never expire** — set an expiry on the replacement |
