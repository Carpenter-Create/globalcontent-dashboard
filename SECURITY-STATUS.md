...
| **`migration-drift.yml` secrets — BOTH HAVE BEEN SET SINCE 2026-07-22** | **This file said "Not set. The workflow no-ops" for five days. That was false**, and `out-of-repo-results.md` A2 carried the same error. `gh secret list` shows `SUPABASE_ACCESS_TOKEN` and `SUPABASE_DB_PASSWORD`, both dated `2026-07-22T03:5x`. The workflow has been running against production the whole time and *working* — the `05:08` push-to-main run on 2026-07-27 correctly failed with prod behind by seven. Consequences: (a) the account-scoped token has been in repo secrets for five days and is **now unused** by any workflow — **delete it**; (b) the `postgres` password is already in GitHub, so `drift_reader` is a **rotation**, not an avoidance |
